...
#         b2.append(tf.Variable(tf.random_uniform([output_dim],-a/sqrt(hidden_layer_dim),a/sqrt(hidden_layer_dim))))
	...
	W6.append(tf.Variable(tf.random_uniform([hidden_layer_dim,output_dim],-b/sqrt(hidden_layer_dim),b/sqrt(hidden_layer_dim))))
	b6.append(tf.Variable(tf.random_uniform([output_dim],-b/sqrt(hidden_layer_dim),b/sqrt(hidden_layer_dim))))
...
pickle.dump(cnt, open( str(idx)+"_cnt_"+str(N*paral_nets)+"_7_"+str(hidden_layer_dim)+"x5_1_allinputs_relu.p", "wb" ), -1)
