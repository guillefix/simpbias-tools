...
for i in range(paral_nets):
    ...
    with tf.variable_scope(scope_name):
        ...
        W1.append(tf.random_normal([input_dim,hidden_layer_dim],stddev=sigmaw/sqrt(input_dim)))
        b1.append(tf.random_normal([hidden_layer_dim],stddev=sigmab))

        W2.append(tf.random_normal([hidden_layer_dim,hidden_layer2_dim],stddev=sigmaw/sqrt(hidden_layer_dim)))
        b2.append(tf.random_normal([hidden_layer2_dim],stddev=sigmab))

        W3.append(tf.random_normal([hidden_layer2_dim,output_dim],stddev=sigmaw/sqrt(hidden_layer2_dim)))
        b3.append(tf.random_normal([output_dim],stddev=sigmab))
...
pickle.dump(cnt, open( str(idx)+"_cnt_"+str(N*paral_nets)+"_"+str(input_dim)+"_"+str(hidden_layer_dim)+"_"+str(hidden_layer2_dim)+"_"+str(output_dim)+"_"+str(sigmaw)+"_"+str(sigmab)+"_gaussian_relu.p", "wb" ), -1)
